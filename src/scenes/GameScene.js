...
const LAYER_SURFACE_Y_OVERRIDE = {
    // 2026-06-04 인게임 검수: 사장님 "레벨2,4 지표면선이 실제보다 조금 아래".
    //   원인(자산 밝기 오프라인 측정으로 확정): 띠구간 max-drop(win30)이
    //   "지표면 아래 2차 어두워짐"에 끌려 내려감.
    //   - layer_002: 밝은 모래밭 끝 row 824(밝기 207→147)이 진짜 지표면이나,
    //                890~900 2차 하강(→69)을 argmax(889)가 잡음 → 824로 고정.
    //   - layer_004: 타일바닥 끝 검은경계 row 800(밝기 149→12)이 지표면이나,
    //                아래 흙·물결선 탓에 argmax(865)가 내려감 → 800으로 고정.
    //   나머지(001/003/005/006)는 argmax가 지표면과 일치 → 자동측정 유지.
    'layer_002': 824,
    'layer_004': 800,
};
